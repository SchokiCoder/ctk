...
#define CTK_MAJOR_VERSION 0
#define CTK_MINOR_VERSION 4
#define CTK_PATCH_VERSION 0

/* Configuration defines
 */

#ifndef CTK_DEFAULT_FONTSIZE
#define CTK_DEFAULT_FONTSIZE 12
#endif

#ifndef CTK_DEFAULT_WINDOW_FLAGS
#define CTK_DEFAULT_WINDOW_FLAGS (SDL_WINDOW_RESIZABLE)
#endif

#ifndef CTK_DEFAULT_MAX_FRAMERATE
#define CTK_DEFAULT_MAX_FRAMERATE 60
#endif

#ifndef CTK_INSTANCE_MAX_MENUS
#define CTK_INSTANCE_MAX_MENUS 32
#endif
...
typedef struct CTK_WidgetStyle {
	...
	void              (*render)(struct CTK_Instance*, const CTK_WidgetId);
} CTK_WidgetStyle;
...
typedef struct CTK_Style {
	...
	/* These assume RenderTarget to be set to its inst's content texture. */
	void            (*draw_menu)(struct CTK_Instance*, const CTK_MenuId);
	void            (*draw_menubar)(struct CTK_Instance*);
} CTK_Style;
...
typedef struct CTK_Menubar {
	...
	CTK_MenuId  menu[CTK_MENUBAR_MAX_CASCADES];
...
typedef struct CTK_Instance {
	...
	size_t          menus;
	CTK_Menu        menu[CTK_INSTANCE_MAX_MENUS];
	CTK_Menubar     menubar;
	bool            redraw;
	CTK_Style       style;
	TTF_TextEngine *tengine;
	bool            txt_input_suspended;
	CTK_MenuId      visible_menu;
	SDL_Window     *win;
...
void
CTK_RenderWidget(CTK_Instance       *inst,
...
CTK_AddMenu(CTK_Instance *inst)
{
	...
	inst->menu[m].focused_cmd = -1;
	inst->menu[m].rect.x = 0;
...
CTK_AddMenubar(CTK_Instance *inst)
{
	...
	inst->menubar.h = 0;
	inst->menubar.hovered_casc = -1;
	inst->menubar.cascades = 0;
                      ...
                      const size_t      underline)
{
	...
	if ((size_t) h > mb->h) {
		...
		for (i = 0; i < mb->cascades; i++) {
			inst->menu[mb->menu[i]].rect.y = mb->h;
		}
	}
                 ...
                 const CTK_WidgetId  widget)
{
	...
	inst->wstyle[widget].render(inst, widget);
	inst->redraw = true;
	SDL_SetRenderTarget(r, NULL);
}
                  ...
                  const SDL_KeyboardEvent  e)
{
	...
	if (inst->visible_menu < CTK_INSTANCE_MAX_MENUS &&
		...
		for (i = 0; i < menu->cmds; i++) {
			...
			temp = menu->label[i]->text[menu->underline[i]];
			temp = toupper(temp);
	...
	case SDLK_MENU:
		fw = CTK_GetFocusedWidget(inst);
		menu = &inst->menu[inst->entry_menu];

		if (CTK_WTYPE_ENTRY != inst->type[fw])
			break;

		menu->rect.x = inst->rect[fw].x;
		menu->rect.y = inst->rect[fw].y + inst->rect[fw].h;
		if (inst->menubar.h > 0) {
			menu->rect.y += inst->menubar.h;
		}

		if (inst->cursor[fw] != inst->selection[fw]) {
			all_cmds = true;
		}
		menu->enabled[inst->entry_menu_cut] = all_cmds;
		menu->enabled[inst->entry_menu_copy] = all_cmds;
                  ...
                  const SDL_MouseButtonEvent  e)
{
	...
	if (inst->visible_menu < CTK_INSTANCE_MAX_MENUS) {
		p.x = e.x;
		p.y = e.y;
		if (!SDL_PointInRectFloat(&p,
		                          &inst->menu[inst->visible_menu].rect)) {
			CTK_UnfocusMenubar(inst);
		}
		return;
	}
                      ...
                      const SDL_MouseMotionEvent  e)
{
	...
	if (inst->hovered_w < inst->widgets)
		CTK_RenderWidget(inst, inst->hovered_w);

	if (old_hov_wid  < inst->widgets)
		CTK_RenderWidget(inst, old_hov_wid);
}
